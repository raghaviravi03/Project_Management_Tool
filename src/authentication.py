...
def display_login_page():
    ...
    signup_redirect = st.session_state.get('signup_redirect', True)
        ...
        with st.form(key='login_form', clear_on_submit=True):
            ...
            if login_btn:
                ...
                if user:
                    ...
                    st.session_state['is_first_login'] = 'is_first_login' in st.session_state
                    st.rerun()
                else:
                    st.error("Invalid email or password, or no account exists for the particular user.")
    ...
    if signup_redirect:
        with col2:
            st.subheader("Signup")
            st.write("If you're using this tool for the first time, please register yourself or wait for the admin to sign you up!")
